...
object Inference {
    ...
    suspend fun run(
        ...
        try {
            ...
            // Encode SNI to features (include latency/throughput in encoding)
    ...
    private fun applyBiases(probs: FloatArray, biases: FloatArray): FloatArray {
        ...
        // Add biases and renormalize
        val biased = probs.mapIndexed { i, prob -> prob + biases[i] }
        val min = biased.minOrNull() ?: 0f
        val normalized = biased.map { (it - min).coerceAtLeast(0f) }
        val sum = normalized.sum()
        
        return if (sum > 0f) {
            normalized.map { (it / sum).toFloat() }.toFloatArray()
        } else {
